...
def interleave_examples(reader, batch_size: int = 1, context_size: int = 1,
    ...
    Args:
        ...
        context_size (int): Size in sentences of the 'text' field.
        label_size (int):  Size in sentences of the 'label' field.
        step_size (int):  Sliding window step.
    """
    # Iterate over a batch of the text.
    ...
    for episode in reader:
        ...
        if len(sentences) <= context_size + label_size + step_size:
            continue

        windowed_sentences = list(
            more_itertools.windowed(sentences, n=context_size + label_size, fillvalue=" ", step=step_size))

        example_list = deque([])

        for i, window in enumerate(windowed_sentences):
            context = window[context_size:]
            label = window[:-label_size]
            example = {
                "id": f"{episode['id']}-{i}",
